...
__SHELL=0 \
   ...
   x_AWS__SES_TO=3600 \
    ...
    _GSM__SEC_PATH='...gsmSecPath...' \
    _GSM__SEC_VAL__USR='...gsmSecValUsr...' \
    _GSM__SEC_VAL__PWD='...gsmSecValPwd...' \
...
scrEOF
        ...
        [ -z "${_GSM__SEC_COL}" ] || [ -z "${_GSM__SEC_PATH}" ] || [ \
            -z "${_GSM__SEC_VAL__USR}" ] || [ -z "${_GSM__SEC_VAL__PWD}" \
        ] || ( set +x
            function gsm-cli () {
                podman run --rm -i \
                    -u 0:0 \
            ...
            gsmSecExist=$(gsm-cli "$(cat - 0<<scrEOF
python3 main.py list \
    -c ${_GSM__SEC_COL@Q} \
    -o json
scrEOF
            )" | jq -r --arg p "${_GSM__SEC_PATH}" '
                (index($p) // -1) + 1
            ') 2>/dev/null || {
                echo \
                    'You do NOT have access to GSM Secret'\
                    "\`${_GSM__SEC_PATH}\` in Collection \`${_GSM__SEC_COL}\`."\
                    1>&2
                exit 1
            }
...
scrEOF
              ...
              (1)
                _GSM__SEC_VAL="$(jq -cn \
                    --arg ku "${_GSM__SEC_VAL__USR}" \
                    --arg vu "${crdUsr}" \
                    --arg kp "${_GSM__SEC_VAL__PWD}" \
                    --arg vp "${crdPwd}" \
                    '{($ku): $vu, ($kp): $vp}'
                )"
                {
