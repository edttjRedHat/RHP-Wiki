...
__SHELL=0 \
   ...
   x_AWS__SES_TO=3600 \
    ...
    bash -c "$(cat - 0<<'cmdEOF'
        ...
        [ -z "${bwData}" ] && {
            echo "You do not have access to BitWarden Note \`${_BW__NOTE_NAME}\`." 1>&2
            exit 1
        }
        ...
        while read -r iamRoleName; do
            ...
            [ "$(
                aws iam get-role --role-name "${iamRoleName}" \
                    --output text --query "Role.AssumeRolePolicyDocument.Statement[?((Effect == 'Allow') && contains(Principal.Service, 'ec2.amazonaws.com'))]"
            )" = '' ] && continue
            ...
            if ((__DRY_RUN)); then
                ...
                while read -r e; do
                    ...
                    aws iam remove-role-from-instance-profile --instance-profile-name "${e}" --role-name "${iamRoleName}"
                done 0< <(
                ...
                while read -r e; do
                    ...
                    aws iam delete-instance-profile --instance-profile-name "${e}"
                done 0< <(
                    aws iam list-instance-profiles \
                ...
                while read -r e; do
                    ...
                    aws iam detach-role-policy --role-name "${iamRoleName}" --policy-arn "${e}"
                done 0< <(
                ...
                while read -r e; do
                    ...
                    aws iam delete-role-policy --role-name "${iamRoleName}" --policy-name "${e}"
                done 0< <(
