...
__SHELL=0 \
    ...
    _OCP__CLUSTER_DIR=ocp-clusters/rhp/tmp/aws \
    _OCP__INSTLR_LOG_LEVEL=info \
    _OCP__INSTLR_DIR=ocp-install/4.19/aws/arm \
    _AWS__USE_SSO=0 \
   ...
   x_AWS__SES_TO=3600 \
    ...
    BW_SESSION="${BW_SESSION:+$([ -f "${BW_SESSION}" ] && cat "${BW_SESSION}" || ${BW_SESSION})}" \
    BW_SESSION="${BW_SESSION:-$(bw unlock --raw || bw login --raw)}" \
    AWS_REGION=us-east-1 \
    AWS_ACCOUNT_ID=624914081466 \
    AWS_CONFIG_FILE="${HOME}/.aws/config" \
    AWS_SHARED_CREDENTIALS_FILE="${HOME}/.aws/credentials" \
    bash -O expand_aliases -c "$(cat - 0<<'cmdEOF'
        ...
        if ((_AWS__USE_SSO)); then
            ...
            eval "$(bw get notes "${_BW__NOTE_NAME}")"
        fi
...
#       eval "$(aws configure export-credentials --format env)"
        ...
        function openshift-install () { \command "${_OCP__INSTLR_DIR}/openshift-install" --dir "${_OCP__CLUSTER_DIR}/" ${_OCP__INSTLR_LOG_LEVEL:+--log-level "${_OCP__INSTLR_LOG_LEVEL}"} "$@"; }
        case ${_OCP__INSTLR_ACTION} in
          (-2|0|2)  openshift-install destroy bootstrap;;&
          (-2|-1|2) openshift-install destroy cluster;;&
          (-2|2)    rm -rf "${_OCP__CLUSTER_DIR}/";;&
          (1|2)
            openshift-install create cluster
            cp "${_OCP__CLUSTER_DIR}/auth/kubeconfig" "${_OCP__CLUSTER_DIR}/"
            mv "${_OCP__CLUSTER_DIR}/auth/kubeconfig" "${_OCP__CLUSTER_DIR}/auth/kubecfg"
            ;;
          (i)
            # Interactive Session.
...
## Post Installation
<details>
    <summary>Install AWS EC2 SSM Agent on Nodes</summary>
